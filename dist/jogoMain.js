...
const Chefes_1 = require("./Chefes");
const inventario_1 = require("./inventario");
const inventario_2 = require("./inventario");
// Perguntas
const nomeJ = readline_sync_1.default.question("Me diga o seu nome de Jogador: ");
const idadeJ = Number(readline_sync_1.default.question("Qual a sua idade de Jogador? "));
const classeJ = readline_sync_1.default.question("Me diga a sua classe: ");
const hab1 = readline_sync_1.default.question("Me diga qual é a sua primeira habilidade: ");
const hab2 = readline_sync_1.default.question("Me diga qual a sua segunda habilidade: ");
let inventario1 = new inventario_1.Inventario();
// Cria jogador
let Jogador1 = new Personagem_1.Jogador(nomeJ, idadeJ, classeJ, hab1, hab2, 5, 105, 0);
// Cria inimigos normais
const Creeper = new Inimigos_1.Inimigos("Creeper", 40, 26);
const Esqueleto = new Inimigos_1.Inimigos("Esqueleto", 20, 15);
const Harpia = new Inimigos_1.Inimigos("Harpia", 15, 9);
const Gigante = new Inimigos_1.Inimigos("Gigante", 70, 40);
const Zombie = new Inimigos_1.Inimigos("Zombie", 50, 20);
const Minotauro = new Inimigos_1.Inimigos("Minotauro", 69, 25);
const Aranha = new Inimigos_1.Inimigos("Aranha de 7 patas", 35, 10);
// Cria Subchefes
let Ciclope = new SubChefes_1.SubChefe("Raio laser", "Fogo", 10, 8, "Ataque de árvore", "Ciclope", 90, 45);
let Cidy = new SubChefes_1.SubChefe("Garras sangrentas", "Rugido sonolento", 20, 60, "Abraço explosivo", "Cid", 100, 30);
let Enderman = new SubChefes_1.SubChefe("Encarada mortal", "Bloco de grama", 35, 10, "Olho do ender", "Enderman", 90, 40);
// Cria Chefe
let Kraken = new Chefes_1.Chefe("Mil tentáculos", "Tsunami", 55, 100, "Cruzeiro nevoso", "Kraken", 300, 60, "Tentáculo Chicote", "Tentáculo Giratório", 10, 50);
// enredo part one
// história aos poucos
async function historia(texto, velocidade = 75) {
    let acumuladoo = "";
...
async function main() {
    ...
    `);
        ...
        `);
        const resposta1 = Number(readline_sync_1.default.question("Escolha a sua alternativa: (1 ou 2)"));
...
[Eles chegam até as armas]
    ...
    `);
        const resposta2 = Number(readline_sync_1.default.question(`Voce ve 5 itens a sua frente. Escolha 1 deles:
        ...
        while (![1, 2, 3, 4, 5].includes(resposta2)) {
            const resposta2 = Number(readline_sync_1.default.question(`\n\nVoce ve 5 itens a sua frente. Escolha 1 deles:
        ...
        switch (resposta2) {
            case 1:
                const tocha = new inventario_2.Itens(["tocha"], "Tocha");
                inventario1.adicionarItem(tocha);
                break;
            case 2:
                const pocaoEnvenena = new inventario_2.Itens(["Poção de envenenamento"], "Poção de veneno");
                inventario1.adicionarItem(pocaoEnvenena);
                break;
            case 3:
                const armadura = new inventario_2.Itens(["armadura"], "armadura");
                inventario1.adicionarItem(armadura);
                break;
            case 4:
                const arcoFlexa = new inventario_2.Itens(["Arco e flecha"], "Arco e flecha");
                inventario1.adicionarItem(arcoFlexa);
                break;
        }
        await historia(`Ótima escolha. Agora você possui o ${inventario1.verInventario()} para te ajudar em sua aventura!
    .
    .
    .
    ...
    `);
        ...
        switch (sorteio1) {
            ...
            case 4:
                ...
                break;
            case 5:
                const shuriken = new inventario_2.Itens(["Shuriken"], "Shuriken");
    ...
    `);
        const resposta3 = Number(readline_sync_1.default.question(`\n Esolha uma das opções (1, 2 ou 3)`));
    ...
    Você não consegue enxergar nada além de árvores e mais árvores. Você observa e sente que está sendo observado. Próximo a uma pedra grande, você observa uma pequena luz estranha saindo sobre ela. Cabe a você decidir se aproximar ou não`);
        let a = true;
        while (a === true) {
            const resposta4 = Number(readline_sync_1.default.question(`Você decide. Escolha '1' para sim ou '2' para não`));
            if (resposta4 === 1) {
        ...
        .`);
                ...
                if (dado >= 10) {
                    ...
                    const pocaoCura = new inventario_2.Itens(["Poção de cura"], "Poção de cura");
    ...
    `);
        ...
        while (Harpia.vida > 0 && Jogador1.vida > 0) {
            ...
            let combateJ1 = Number(readline_sync_1.default.question(`
            ...
            switch (combateJ1) {
                case 1:
                    ...
                    let ataqueJ = Number(readline_sync_1.default.question(`
                ...
                case 2:
                    ...
                    let resposta5 = readline_sync_1.default.question(`deseja usar algum item? (sim) ou (nao)`).toLowerCase();
                    if (resposta5 === "sim") {
        ...
        if (Jogador1.vida <= 0) {
            ...
            let continuar = readline_sync_1.default.question(`Responda 'S' ´para reiniciar a partida ou 'N' para fechar o jogo`).toLowerCase();
            if (continuar === "s") {
            ...
            await historia(`... Você conseguiu derrotar a bendita besta.... mas não sabe ainda oq ela era.`);
...
A floresta se torna mais densa. À sua frente, três trilhas distintas surgem entre as árvores.`);
        ...
        let escolhaCaminho = Number(readline_sync_1.default.question("Escolha a sua trilha: (1, 2 ou 3) "));
        if (escolhaCaminho === 1) {
...
Ao se abaixar para pegar, uma corda estala! Você é içado rapidamente por uma rede!`);
            ...
            while (Esqueleto.vida > 0 && Jogador1.vida > 0) {
                ...
                let combateJ1 = Number(readline_sync_1.default.question("1: Atacar | 2: Inventario | 3: Defender "));
                switch (combateJ1) {
                    case 1:
                        console.log(`\n\nEscolha o seu ataque: `);
                        let ataqueJ = Number(readline_sync_1.default.question(`
                    ...
                    `));
                        ...
                        // turno do inimigo
                        Harpia.atacar();
                        console.log(`\n O inimigo atacou com ${Esqueleto.dano}`);
                        Jogador1.vida -= Esqueleto.dano;
                        console.log(`vida inimigo: ${Esqueleto.vida}`);
                        break;
                    case 2:
                        console.log(`Seus itens:`);
                        inventario1.verInventario();
                ...
                Jogador1.vida -= Esqueleto.atacar();
                console.log(`O jogador tomou ${Esqueleto.atacar()} de dano!`);
            }
        }
            ...
            let abrirBau = Number(readline_sync_1.default.question("1: Abrir baú | 2: Ignorar e seguir "));
            if (abrirBau === 1) {
                const pocaoForca = new inventario_2.Itens(["Poção de Força"], "Poção de Força");
        ...
        await historia(`[Você abriu o baú e equipou um Capacete de Ferro! +240 HP]`);
        Jogador1.vida += 240;
        ...
        while ((inimigosVila.some(i => i.vida > 0) || Esqueleto.vida > 0 || Zombie.vida > 0) && Jogador1.vida > 0) {
            console.log(`\n--- BATALHA NO CENTRO ---`);
            inimigosVila.forEach((ini, idx) => { if (ini.vida > 0)
                console.log(`${idx + 1}: ${ini.nome} (${ini.vida}hp)`); });
            let alvo = Number(readline_sync_1.default.question("Atacar quem? (1 ou 2) "));
            if (inimigosVila[alvo - 1]) {
                ...
                Escolha um dos números acima`));
                    ...
                    `));
                            Jogador1.atacar(ataqueJ);
                            Zombie.vida -= Jogador1.dano;
                            // turno do inimigo
                            Zombie.atacar();
                            console.log(`\n O inimigo atacou com ${Zombie.dano}`);
                            Jogador1.vida -= Zombie.dano;
                            console.log(`vida inimigo: ${Zombie.vida}`);
                            break;
                        case 2:
                            console.log(`Seus itens:`);
                            inventario1.verInventario();
                ...
                Jogador1.vida -= Esqueleto.atacar();
                Jogador1.vida -= Zombie.atacar();
                console.log(`O jogador tomou ${Esqueleto.atacar()} de dano!`);
                console.log(`O jogador tomou ${Zombie.atacar()}`);
            }
            // --- UPGRADE E FUGA ---
            ...
            while ((hordaFinal.some(i => i.vida > 0)) && Jogador1.vida > 0) {
                ...
                hordaFinal.forEach((ini, idx) => { if (ini.vida > 0)
                    console.log(`${idx}: ${ini.nome} (${ini.vida}hp)`); });
                let alvo = Number(readline_sync_1.default.question("Escolha o alvo (1, 2 ou 3): "));
                ...
                Escolha um dos números acima`));
                    ...
                    `));
                                Jogador1.atacar(ataqueJ);
                                Creeper.vida -= Jogador1.dano;
                                // turno do inimigo
                                Creeper.atacar();
                                console.log(`\n O inimigo atacou com ${Creeper.dano}`);
                                Jogador1.vida -= Creeper.dano;
                                console.log(`vida inimigo: ${Creeper.vida}`);
                                break;
                            case 2:
                                console.log(`Seus itens:`);
                                inventario1.verInventario();
                ...
                Escolha um dos números acima`));
                    ...
                    `));
                                Jogador1.atacar(ataqueJ);
                                Gigante.vida -= Jogador1.dano;
                                // turno do inimigo
                                Gigante.atacar();
                                console.log(`\n O inimigo atacou com ${Gigante.dano}`);
                                Jogador1.vida -= Gigante.dano;
                                console.log(`vida inimigo: ${Gigante.vida}`);
                                break;
                            case 2:
                                console.log(`Seus itens:`);
                                inventario1.verInventario();
                ...
                Jogador1.vida -= Gigante.atacar();
                Jogador1.vida -= Zombie.atacar();
                Jogador1.vida -= Creeper.atacar();
                console.log(`O jogador tomou ${Esqueleto.atacar()} de dano!`);
                console.log(`O jogador tomou ${Zombie.atacar()}`);
                console.log(`O jogador tomou ${Gigante.atacar()}`);
            }
        }
    }
    // --- FINALIZAÇÃO ---
